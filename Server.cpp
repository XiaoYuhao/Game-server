...
void Server::login(login_ask_package &p){
	...
	if(res!=1){//登录错误
		...
		sprintf(temp_log,"Login Failed. Requset from IP:%s Port:%d",inet_ntoa(client.sin_addr),htons(client.sin_port));
		print_log(temp_log);
	}
	else{	//登录成功
		if(id_session.count(userid)){			//此userid已存在session
			...
			memcpy(sendbuf,&rp,sizeof(rp));
			send_package(sendbuf,sizeof(rp));
			sprintf(temp_log,"Login success(Userid:%d). Requset from IP:%s Port:%d",userid,inet_ntoa(client.sin_addr),htons(client.sin_port));
			print_log(temp_log);
		}
	}
}
